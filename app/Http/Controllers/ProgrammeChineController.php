...
class ProgrammeChineController extends Controller
{
    ...
    /**
     * Retourne les données nécessaires au front pour Chine
     */
    public function data()
    {
        // Récupération des chauffeurs pour agence Chine (ID 9)
        $rawChauffeurs = User::where('agence_id', 9)
        ...
        // Récupération des programmes (INCLUANT "à planifié") pour agence Chine
        ...
        // Récupérer uniquement les dépôts EFFECTUÉS pour récupération
    ...
    public function createDepot(Request $request)
    {
        try {
            ...
            DB::rollBack();
            Log::error("Erreur création dépôt Chine: " . $e->getMessage());
     ...
     /**
     * Création de plusieurs dépôts en une fois
     */
...
$dateAvecHeure = $request->date_programme . ' ' . now()->format('H:i:s');
            foreach ($request->programmes as $index => $programmeData) {
                try {
                    ...
                    Log::info("✅ Dépôt multiple Chine créé avec ID: " . $programme->id . " - Référence: " . $referenceGeneree);
                    ...
                    Log::error("❌ Erreur création dépôt multiple Chine {$index}: " . $e->getMessage());
                }
            }
            ...
            DB::rollBack();
            Log::error("❌ Erreur création dépôts multiples Chine: " . $e->getMessage());
    ...
    /**
     * Création d'une récupération simple
     */
    public function createRecuperation(Request $request)
    {
        try {
            ...
            $programme = Programme::create([
                ...
                'tel_expediteur'    => $request->tel_expediteur,
                'nature_du_colis'   => $request->nature_du_colis,
                'mode_transit'      => $request->mode_transit ?? 'aerien',
                'agence_expedition' => 'Agence de Chine',
                'devise'            => 'FCFA',
                'etat_rdv'          => 'en attente',
            ]);

            Log::info("✅ Programme Chine créé avec ID: " . $programme->id . " - Référence: " . $referenceGeneree);
            ...
            if ($request->type_reference === 'devis' && $modificationsApportees && $request->has('devis_items')) {
                ...
                if (!empty($devisItemsData)) {
                    Log::info("📦 Création de nouveaux articles pour le programme Chine ID: " . $programme->id);
                    ...
                    Log::info("✅ " . count($devisItemsData) . " articles créés avec succès pour le programme Chine ID: " . $programme->id);
                }
            }
            ...
            elseif ($request->type_reference === 'devis' && !$modificationsApportees) {
                ...
                if ($devis && $devis->items->count() > 0) {
                    Log::info("📦 Copie des articles du devis original pour le programme Chine ID: " . $programme->id);
            ...
            // Cas 3: Types "depot" ou "manuel" - UTILISER les articles modifiés/ajoutés
                ...
                if ($modificationsApportees && $request->has('devis_items')) {
                    ...
                    if (!empty($devisItemsData)) {
                        Log::info("📦 Création d'articles personnalisés pour le programme Chine ID: " . $programme->id);
                        ...
                        $devisItemsCreated = true;
                        Log::info("✅ " . count($devisItemsData) . " articles personnalisés créés pour le programme Chine ID: " . $programme->id);
                    }
                } else {
                    // Si pas de modifications, créer un seul article avec les infos du formulaire
                    Log::info("📦 Création d'un article par défaut pour le programme Chine ID: " . $programme->id);
            ...
            DB::rollBack();
            Log::error("❌ Erreur création récupération Chine: " . $e->getMessage());
        ...
        try {
            ...
            foreach ($request->programmes as $index => $programmeData) {
                try {
                    ...
                    $modeTransit = 'aerien';
                    $agenceExpedition = 'Agence de Chine';
                    $emailExpediteur = '';
                    $devise = 'FCFA';
                    $agenceDestination = '';
                    $prenomExpediteur = '';
                    ...
                    if ($typeReference === 'devis') {
                        ...
                        if ($devis) {
                            ...
                            $agenceExpedition = $devis->agence_expedition ?? 'Agence de Chine';
                            $emailExpediteur = $devis->email_expediteur ?? '';
                            $devise = $devis->devise ?? 'FCFA';
                    ...
                    Log::info("✅ Programme multiple Chine créé avec ID: " . $programme->id . " - Référence: " . $referenceGeneree);
                    ...
                    if ($typeReference === 'devis' && $modificationsApportees && isset($programmeData['devis_items'])) {
                        ...
                        if (!empty($devisItemsData)) {
                            Log::info("📦 Création de nouveaux articles pour le programme multiple Chine ID: " . $programme->id);
                            ...
                            $devisItemsCreated = true;
                            Log::info("✅ " . count($devisItemsData) . " articles créés avec succès pour le programme multiple Chine ID: " . $programme->id);
                        }
                    }
                    // Cas 2: Type "devis" SANS modifications - COPIER les items du devis original
                    elseif ($typeReference === 'devis' && !$modificationsApportees) {
                        ...
                        if ($devis && $devis->items->count() > 0) {
                            Log::info("📦 Copie des articles du devis original pour le programme multiple Chine ID: " . $programme->id);
                            ...
                            $devisItemsCreated = true;
                            Log::info("✅ " . $devis->items->count() . " articles copiés du devis original pour le programme multiple Chine");
                        }
                    }
                    // Cas 3: Types "depot" ou "manuel" - UTILISER les articles modifiés/ajoutés
                    else {
                        ...
                        if ($modificationsApportees && isset($programmeData['devis_items'])) {
                            ...
                            if (!empty($devisItemsData)) {
                                Log::info("📦 Création d'articles personnalisés pour le programme multiple Chine ID: " . $programme->id);
                                ...
                                $devisItemsCreated = true;
                                Log::info("✅ " . count($devisItemsData) . " articles personnalisés créés pour le programme multiple Chine ID: " . $programme->id);
                            }
                        } else {
                            // Si pas de modifications, créer un seul article avec les infos du formulaire
                            Log::info("📦 Création d'un article par défaut pour le programme multiple Chine ID: " . $programme->id);
                    ...
                    Log::info("✅ Programme multiple Chine traité avec succès: " . $referenceGeneree);
                    ...
                    Log::error("❌ Erreur création programme multiple Chine {$index}: " . $e->getMessage());
                }
            }
            ...
            DB::rollBack();
            Log::error("❌ Erreur création programmes multiples Chine: " . $e->getMessage());
  ...
  /**
    ...
    {
        try {
            Log::info("Recherche référence Chine: " . $reference);
            ...
            if ($devis) {
                ...
                Log::info("Devis confirmé Chine trouvé: " . $devis->reference);
            ...
            if ($depot) {
                ...
                Log::info("Dépôt effectué Chine trouvé: " . $depot->reference_generee . " - État: " . $depot->etat_rdv);
            ...
            // Vérifier aussi si c'est un dépôt en attente pour informer l'utilisateur
            ...
            if ($depotEnAttente) {
                Log::info("Dépôt en attente Chine trouvé: " . $depotEnAttente->reference_generee);
            ...
            Log::info("Référence non trouvée Chine, considérée comme manuelle: " . $reference);
            return response()->json([
                'type' => 'manuel',
                'existe' => false,
                'valide' => false,
            ...
            Log::error("Erreur getReferenceInfo Chine: " . $e->getMessage());
            return response()->json([
                'error' => $e->getMessage(),
                'existe' => false,
                'valide' => false
  ...
  /**
    ...
    /**
     * Vérifie les items d'un programme
     */
    ...
    public function createRecuperationFromDevis(Request $request)
    {
        try {
            ...
            $programme = Programme::create([
                ...
                'tel_expediteur'    => $request->tel_expediteur,
                'nature_du_colis'   => $request->nature_du_colis,
                'mode_transit'      => $devis->mode_transit ?? 'aerien',
                'agence_expedition' => 'Agence de Chine',
                'devise'            => 'FCFA',
                'etat_rdv'          => 'en attente',
            ]);
            ...
            DB::rollBack();
            Log::error("❌ Erreur création récupération depuis devis Chine: " . $e->getMessage());
    ...
    public function destroy(Programme $programme)
    {
        try {
            ...
            Log::error("Erreur de suppression du programme Chine #{$programme->id}: " . $e->getMessage());
...
{
    try {
        // Pour Chine, vous devrez adapter l'agence_id (probablement 6 ou autre)
        $rawChauffeurs = User::where('agence_id', 9) // À adapter pour Chine
            ->where('role', 'chauffeur')
            ->where('is_active', true)
            ->get(['id', 'first_name', 'last_name']);
...
{
    return view('AGENCE_CHINE.transport.depot');
}

public function showRecuperationPage()
{
    return view('AGENCE_CHINE.transport.recuperation');
}

public function showLivraisonPage()
{
    return view('AGENCE_CHINE.transport.livraison');
}
public function showEdit($slug)
{
    try {
        ...
        Log::info("🔍 Tentative de chargement du programme Chine ID: " . $id_numerique);
       
        // Charger les ProgrammeItems
        ...
        // Convertir la date en objet Carbon si c'est une string
        ...
        Log::info("✅ Programme Chine trouvé: " . $programme->reference_generee . " - État: " . $programme->etat_rdv);
        Log::info("📦 Nombre d'articles trouvés: " . ($programme->items ? $programme->items->count() : 0));
        
        // Récupérer les chauffeurs
        $chauffeurs = User::where('agence_id', 9)
            ->where('role', 'chauffeur')
            ->where('is_active', true)
            ->get(['id', 'first_name', 'last_name']);
        
        return view('AGENCE_CHINE.transport.planingedit', compact('programme', 'chauffeurs'));
        
    } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
        Log::error("❌ Programme Chine non trouvé avec ID: " . ($id_numerique ?? 'N/A'));
        return redirect()->route('chine_programme.planing.index')
            ->with('error', 'Programme non trouvé ou déjà effectué');
            
    } catch (\Exception $e) {
        Log::error("❌ Erreur affichage page édition Chine: " . $e->getMessage());
        Log::error("Stack trace: " . $e->getTraceAsString());
...
public function updateProgramme(Request $request, $id)
{
    ...
    Log::info("🔄 Début de la mise à jour du programme Chine ID: " . $id_numerique);
    ...
    try {
        ...
        Log::info("✅ Programme principal Chine ID " . $programme->id . " mis à jour.");
        ...
        Log::info("✅✅ Mise à jour complète du programme Chine " . $programme->reference_generee . " réussie !");

        return redirect()->route('chine_programme.planing.index')->with('success','Programme modifié avec succès');

    } catch (\Exception $e) {
        DB::rollBack();
        Log::error("❌ Erreur lors de la mise à jour du programme Chine: " . $e->getMessage());
        Log::error("Stack trace: " . $e->getTraceAsString());
...
/**
 * Mettre à jour les items d'un programme
 */
public function updateProgrammeItems(Request $request, $id)
{
    try {
        ...
        DB::rollBack();
        Log::error("❌ Erreur mise à jour articles Chine: " . $e->getMessage());
...
/**
 * Supprimer un item de programme
 */
public function deleteProgrammeItem($programmeId, $itemId)
{
    try {
        ...
        Log::error("❌ Erreur suppression article Chine: " . $e->getMessage());
        return response()->json([
            'success' => false,
...
/**
 * Méthode privée pour gérer la mise à jour des articles
 */
private function updateProgrammeItemsData($programmeId, $itemsData)
{
    try {
        ...
        Log::info("📦 Début mise à jour articles pour programme Chine: " . $programmeId);
        ...
        foreach ($itemsData as $index => $itemData) {
            ...
            if (!$isNewItem) {
                ...
                // Créer un nouvel article
                Log::info("🆕 Création nouvel article pour programme Chine: " . $programmeId);
        ...
        Log::error("❌ Erreur dans updateProgrammeItemsData Chine: " . $e->getMessage());
        Log::error("Stack trace: " . $e->getTraceAsString());
        throw $e;
    }
}
