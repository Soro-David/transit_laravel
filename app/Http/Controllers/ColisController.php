...
public function storeService(Request $request)
{
    ...
    public function storeStep1(Request $request)
    {
        ...
        session(['step1' => $request->only([
            ...
            'adresse_destinataire',
             ...
             'nom_expediteur_societe',
            'prenom_expediteur_societe', 
            'email_expediteur_societe', 
            'tel_expediteur_societe',
            'agence_expedition_societe',
            'adresse_expediteur_societe', 
            'nom_destinataire_societe', 
            'prenom_destinataire_societe',
            'email_destinataire_societe', 
            'tel_destinataire_societe',
            'agence_destination_societe',
            'adresse_destinataire_societe'
        ])]);
    ...
    public function vol_fermer(Request $request)
    {
        ...
        try {
            ...
            $updatedCount = Colis::where('etat', 'Chargé')
                                ->where('mode_transit', 'aerien')
                                ->update(['etat' => 'Fermé', 'reference_vol' => $referenceVol
                            ]);
...
public function generer_qrcode(Request $request, InfobipSmsService $smsService)
{
    ...
    if (!$dernierColis) {
        ...
        // Vérifie l'état du dernier colis
        ...
        // On découpe la référence en parties
        $dernierParts = explode('-', $dernierColis->reference_colis); //AA-051-TC1
        $nouveauParts = explode('-', $reference_colis); //AA-052-TC1


        if (isset($dernierParts[2]) && isset($nouveauParts[2])) {
            $lastSuffixNum = (int) filter_var($dernierParts[2], FILTER_SANITIZE_NUMBER_INT);
            $newSuffixNum  = (int) filter_var($nouveauParts[2], FILTER_SANITIZE_NUMBER_INT);

            // dd($lastSuffixNum, $newSuffixNum );
            // Cas 1 : Le suffixe du nouveau est bien l'incrément du dernier (TC2 après TC1)
            if ($newSuffixNum === $lastSuffixNum + 1) {
                $message = "⚠️ Vous commencez un nouveau conteneur : {$nouveauParts[2]}";
                $referenceColisPrincipale = $reference_colis;
                // dd($lastSuffixNum, $newSuffixNum, $referenceColisPrincipale, $message);

            } else {
                // Cas 2 : Dernier colis fermé → on force l'incrément du suffixe
                if (isset($dernierColis->etat) && $dernierColis->etat === 'Fermé') {
                    $id_reference = 1; // on réinitialise le compteur

                    if (isset($dernierParts[2])) {
                        $prefix = preg_replace('/\d+$/', '', $dernierParts[2]); // récupère "TC"
                        $numero = (int) filter_var($dernierParts[2], FILTER_SANITIZE_NUMBER_INT) + 1; // incrémente le chiffre
                        $dernierParts[2] = $prefix . $numero;
                        $referenceColisPrincipale = implode('-', $dernierParts); // AA-0002-TC2
                    }
    ...
    for ($i = 1; $i <= $quantite_pour_ligne_article; $i++) {
            ...
            try {
                ...
                $filePath = 'qrcodes/colis_' . $safeRef . '_id' . $colisModel->id . '_item' . ($index + 1) . '-' . $i . '.png';
...
//    public function colis arrivés()
    ...
    public function get_tout_colis(Request $request)
    {
        if ($request->ajax()) {
            try {
                // 1. Sélection des colis (la colonne 'devise' est déjà incluse)
                $colis = Colis::select(
                    'colis.id', 'colis.reference_colis', 'colis.produit', 'colis.devise', 
                    'colis.quantite_colis', 'colis.prix_transit_colis', 'colis.montant_service',
                    'colis.expediteur_id', 'colis.destinataire_id', 'colis.etat', 'colis.created_at',
                    'expediteurs.nom as expediteur_nom', 'expediteurs.prenom as expediteur_prenom',
                    'expediteurs.tel as expediteur_tel', 'colis.agence as expediteur_agence',
                ...
                $processedData = $colisGrouped->map(function ($group, $reference) use ($paiements) {
                    ...
                    // Construction du tableau de données, en ajoutant la devise
                    return [
                        'reference_colis' => $reference,
                ...
                // 4. Génération de la réponse DataTables
                return DataTables::of($processedData)
                    ->addColumn('statut_paiement', function ($row) {
                        $status = $row['payment_status'];
                        $devise = htmlspecialchars($row['devise'], ENT_QUOTES, 'UTF-8');
                        $iconClass = ''; $iconColor = ''; $title = '';
                        
                        // Utilisation de la devise dans la bulle d'information
                        $montantPayeFormatted = number_format($row['montant_paye'], 2, ',', ' ') . ' ' . $devise;
                        $prixTotalFormatted = number_format($row['prix_total'], 2, ',', ' ') . ' ' . $devise;
                        ...
                        switch ($status) {
                            ...
                            default:
                                ...
                                $title = 'Impayé (' . $montantPayeFormatted . ' / ' . $prixTotalFormatted . ')';
                                break;
                        }
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        $editUrl = route('colis.valide.show', ['id' => $firstColisId]);
                        $invoiceUrl = route('colis.valide.edit.invoice', ['id' => $firstColisId]);
                        $deleteUrl = route('colis.destroy.colis.valide', ['reference' => $reference]);

                        $editBtn = '<a href="' . $editUrl . '" class="btn btn-sm btn-warning" title="Modifier le colis groupé"><i class="fas fa-edit"></i></a>';
                        
                        // **AJOUT DE data-devise AU BOUTON DE PAIEMENT**
                ...
                Log::error('Erreur dans get_tout_colis: ' . $e->getMessage() . ' à la ligne ' . $e->getLine());
...
public function destroy_colis_valide($reference)
{
    ...
    public function edit_colis_show($id)
    {
        // findOrFail gère automatiquement le cas où le colis n'existe pas (erreur 404).
        $colis_principal = Colis::findOrFail($id);
        ...
        return view('admin.colis.edit_colis_show', [
            'colis_recap_list' => $colis_recap_list,
            'info_partagees'   => $info_partagees,
        ]);
    }
    ...
    public function update_hold(Request $request, InfobipSmsService $infobipSmsService, $id)
    {
        ...
        try {
            ...
            return redirect()->route('colis.hold')->with('success', 'Devis validé et notifié au client avec succès !');
...
{
    return view('admin.colis.devisconfirme');
}

/**
 * Récupère les devis confirmés pour DataTables
 */
...
{
    $devis = Devis::with('items')->findOrFail($id);
    return view('admin.colis.show', compact('devis'));
}

/**
 * Récupère les colis déchargés
 */

/**
 * Récupère les colis en état "Devis" ou "Validé"
 */
...
/**
 * Récupère les colis validés avec statut de paiement
 */
