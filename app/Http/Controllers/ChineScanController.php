...
class ChineScanController extends Controller
{
    ...
    public function get_colis_entrepot(Request $request)
    {
        ...
        try {
            ...
            ->where('colis.agence', 'Agence de Chine') // Condition spécifique à l'agence
            ...
            return DataTables::of($processedData)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $editUrl    = route('chine_colis.valide.edit', ['id' => $row['first_colis_id']]);
                    $invoiceUrl = route('chine_colis.valide.edit.invoice', ['id' => $row['first_colis_id']]);
                    $deleteUrl  = route('chine_colis.destroy.colis.valide', ['reference' => $row['reference_colis']]);
                    $scanUrl    = route('chine_scan.colis.modifier', $row['reference_colis']);
...
{
    if ($request->ajax()) {
        // --- ÉTAPE 1: Récupération des colis ---
        $colis = Colis::select(
                'colis.id',
                'colis.reference_colis', 
                'colis.quantite_colis',
        ...
        // --- ÉTAPE 3: Groupement par référence ---
        $colisGrouped = $colis->groupBy('reference_colis');
        ...
        $processedData = $colisGrouped->map(function ($group) use ($paiements) {
            ...
            return [
                ...
                'destinataire_agence' => $firstColis->destinataire_agence,
                'etat'                => $firstColis->etat,
                'created_at'          => $firstColis->created_at 
                                            ? $firstColis->created_at->format('d/m/Y H:i') 
                                            : 'N/A',
                'payment_status'      => $paymentStatus,
                'prix_total'          => $prixTotalColis,
                'montant_paye'        => $montantTotalPaye,
        ...
        // --- ÉTAPE 4: Retour vers DataTables ---
        return DataTables::of($processedData)
            ->addIndexColumn()
            ->addColumn('action', function($row){
...
public function updateColisEntrepot(Request $request)
{
    ...
    $validated = $request->validate([
        'reference_colis' => 'required|string',
        'id' => 'required', // L'ID peut être un nombre ou une chaîne selon votre BDD
    ]);

    // 2. Recherche du colis spécifique
    $colis = Colis::where('reference_colis', $validated['reference_colis'])
                  ->where('id', $validated['id'])
                  ->first();

    // 3. Gestion du cas où le colis n'est pas trouvé
    if (!$colis) {
        return response()->json([
            'success' => false,
            'message' => 'Aucun colis trouvé avec cette référence et cet identifiant.'
    ...
    // 4. Logique de mise à jour de l'état
    if ($colis->etat === 'En entrepot') {
        // Déjà en entrepôt → on renvoie une info, pas une mise à jour
        return response()->json([
            'success' => false,
            'message' => "ℹ️ Le colis (Réf: {$colis->reference_colis}) est déjà en entrepôt."
        ], 200);

    } elseif ($colis->etat === 'Validé') {
        // Passage normal vers "En entrepôt"
        $colis->etat = 'En entrepot';
        $colis->save();

        return response()->json([
            'success' => true,
            'message' => "✅ Le colis (Réf: {$colis->reference_colis}) a été mis en entrepôt avec succès."
        ], 200);

    } else {
        // Cas où le colis est dans un autre état incompatible
        return response()->json([
            'success' => false,
            'message' => "⛔ Le colis (Réf: {$colis->reference_colis}) ne peut pas être mis en entrepôt. État actuel : '{$colis->etat}'."
        ], 409); // 409 Conflict
    }
}
