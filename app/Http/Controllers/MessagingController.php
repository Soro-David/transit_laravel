...
{
    /**
     * Affiche la page principale d'envoi de messages.
     */
    public function index()
    {
        $containerReferences = Colis::whereNotNull('reference_contenaire')->distinct()->pluck('reference_contenaire');
        $flightReferences = Colis::whereNotNull('reference_vol')->distinct()->pluck('reference_vol');

        return view('admin.client.index', compact('containerReferences', 'flightReferences'));
    }

    /**
     * Fournit les données des clients au format JSON pour la DataTable.
     * Version corrigée avec logique de blocage client.
     */
    public function getClients(Request $request)
    {
        // Hypothèse : Les tables 'expediteurs' et 'destinataires' ont une colonne 'user_id' nullable
        // qui fait référence à la table 'users'.
        $expediteursQuery = Expediteur::select(
        ...
        $destinatairesQuery = Destinataire::select(
            ...
            ->filter(function ($query) use ($request) {
                if ($request->has('search') && !empty($request->search['value'])) {
                    ...
                    $query->collection = $query->collection->filter(function ($item) use ($searchTerm) {
                        // Correction : On vérifie que $item est bien un objet avant d'accéder à ses propriétés
                        if (!is_object($item)) {
                            return false;
                        }
    ...
    // ... Les autres méthodes (sendFilteredMessage, sendIndividualMessage) restent inchangées ...
    /**
    ...
    public function sendFilteredMessage(Request $request)
    {
        ...
        if ($recipientType === 'expediteurs') {
            ...
            $clients = Expediteur::whereIn('id', $clientIds)->get();
        } else {
            $clientIds = $colisQuery->pluck('destinataire_id')->unique();
            $clients = Destinataire::whereIn('id', $clientIds)->get();
        }
    ...
    public function sendIndividualMessage(Request $request)
    {
        ...
        $client = $clientType === 'expediteur'
            ? Expediteur::find($clientId)
            : Destinataire::find($clientId);
