...
<section class="p-4 mx-auto">
    <div class="form-container text-center">
                 ...
                 <div class="card border-0 rounded shadow-sm">
                     ...
                     <div class="card-body p-4">
                         ...
                         <div class="mb-3 d-flex align-items-center">
                             ...
                             <span class="form-control-plaintext w-50"> {{ number_format($totalPrixTransit ?? 0, 0, ',', ' ') }} {{ $first['devise'] ?? 'EUR' }}</span>
                         </div>
                         <div class="mb-3 d-flex align-items-center">
                             <label class="form-label fw-bold w-50">Montant Payé :</label>
                             <span class="form-control-plaintext w-50"> {{ number_format($totalMontantPaye ?? 0, 0, ',', ' ') }} {{ $first['devise'] ?? 'EUR ' }}</span>
                         </div>
                         <div class="mb-3 d-flex align-items-center">
                             <label class="form-label fw-bold w-50">Reste à Payer:</label>
                             <span class="form-control-plaintext w-50 fw-bold {{ ($restePaye ?? 0) > 0 ? 'text-danger' : 'text-success' }}"> {{ number_format($restePaye ?? 0, 0, ',', ' ') }} {{ $first['devise'] ?? 'EUR ' }}</span>
                         </div>
                     </div>
                 </div>
             </div>
        ...
        <div class="d-flex align-items-center gap-2">
            <div>
                <a href="{{ route('aftlb_colis.imprimer.facture', ['id' => $first['id']]) }}" target="_blank" class="btn btn-sm btn-info">
                    <i class="fas fa-file-invoice me-1"></i> Imprimer Facture
                </a>            
            </div>
            <div>
                <a href="{{ route('aftlb_colis.imprimer.bon_livraison', ['id' => $first['id']]) }}" target="_blank" class="btn btn-sm btn-info">
        ...
        <div class="list-group">
            <div class="list-group-item list-group-item-action flex-column align-items-start mb-3 shadow-sm rounded border-0">
                ...
                <div class="mt-2 text-end">
                    ...
                    <a href="{{ route('aftlb_colis.imprimer.etiquette', ['id' => $colis[0]->id]) }}" target="_blank" class="btn btn-sm btn-success me-2">
