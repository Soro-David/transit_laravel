...
$(document).ready(function () {
    var table = $("#devisConfirmeTable").DataTable({
        ...
        ajax: {
            url: '{{ route("aftlb_colis.get.devis.confirmes") }}',
            dataSrc: 'data'
        },
        columns: [
            { 
    ...
    $('#devisConfirmeTable tbody').on('click', '.view-devis-details', function(e) {
        ...
        var detailsUrl = "{{ route('aftlb_colis.devis.details', ['id' => ':id']) }}".replace(':id', devisId);
    ...
    function renderModalContent(data) {
        try {
            ...
            var invoiceUrl = "{{ route('aftlb_colis.valide.edit.invoice', ['id' => ':id']) }}".replace(':id', data.id || '');
