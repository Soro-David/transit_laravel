    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var table = $('#chauffeur-table').DataTable({
                ...
                ajax: {
                    url: '{{ route('aftlb_transport.get.chauffeur.list') }}',  // Route corrigée pour DataTables data
                },
                columns: [{
                        data: 'nom',
                        name: 'nom'
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            var editUrl = `{{ URL::route('aftlb_transport.edit', ['id' => '__ID__']) }}`.replace('__ID__', row.id);
                            var deleteUrl = `{{ URL::route('aftlb_transport.destroy', ['id' => '__ID__']) }}`.replace('__ID__', row.id);
            ...
            $('#chauffeur-table').on('click', '.edit-chauffeur-btn', function() {
                ...
                $.ajax({
                    url: `{{ URL::route('aftlb_transport.edit', ['id' => '__ID__']) }}`.replace('__ID__', chauffeurId),
                    type: 'GET',
            ...
            $('#editChauffeurForm').submit(function(e) {
                ...
                $.ajax({
                    url: `{{ URL::route('aftlb_transport.update', ['id' => '__ID__']) }}`.replace('__ID__', chauffeurId), // Dynamic URL with the chauffeur ID
