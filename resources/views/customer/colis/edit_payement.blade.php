...
<script>
    ...
    console.log("Données de réponse CinetPay:", data); // Affichez l'objet data entier
    if (data.status == "REFUSED") {
        ...
        if (formDataJson.cinetpay_transaction_id) {
            ...
            $.ajax({
                url: '{{ route('customer_colis.store.payement', ['id' => $colis->id]) }}', // Route pour enregistrer le paiement avec l'ID du colis
                method: 'POST',
        ...
        cinetpayButton.on('click', async function() {
    try {
        await loadCinetPay();
        checkout();
    } catch (error) {
        console.error('Erreur de chargement de CinetPay:', error);
        alert('Impossible de charger le service de paiement. Veuillez réessayer.');
    }
});

// Ajouter cette fonction en haut de votre script
...
<input type="hidden" id="colisPrice" value="{{ $colis->prix_transit_colis }}"> {{-- Champ caché pour stocker le prix du colis --}}
{{-- CSS Personnalisé --}}
<style>
