            ...
            @foreach($uniqueClients as $client)
                <tr>
                    ...
                    <td>
                        ...
                        <a href="{{ route('chine_client.edit', ['nom' => $client->nom, 'prenom' => $client->prenom, 'tel' => $client->tel, 'email' => $client->email]) }}" class="btn btn-sm btn-primary">Éditer</a>

                        <form action="{{ route('chine_client.destroy', ['nom' => $client->nom, 'prenom' => $client->prenom, 'tel' => $client->tel, 'email' => $client->email]) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
