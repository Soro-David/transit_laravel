...
@section('content')
    ...
    <form action="{{ route('chine_client.update', ['nom' => $client->nom, 'prenom' => $client->prenom, 'tel' => $client->tel, 'email' => $client->email]) }}" method="POST">
        @csrf
        ...
        <a href="{{ route('chine_client.index') }}" class="btn btn-secondary">Annuler</a>
    </form>
@endsection