...
@section('content')
    ...
    <div class="modal fade" id="ajouter_chauffeur" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <form action="{{ route('chine_transport.chauffeurs.store') }}" method="post">
            @csrf
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header d-flex justify-content-between align-items-center">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var table = $('#chauffeur-table').DataTable({
                ...
                ajax: {
                    url: '{{ route('chine_transport.chauffeurs.data') }}',  // Correct route for DataTables data
                },
                columns: [{
                        data: 'nom',
                        name: 'nom'
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            var editUrl = `{{ URL::route('chine_transport.chauffeurs.edit', ['id' => '__ID__']) }}`.replace('__ID__', row.id);
                            var deleteUrl = `{{ URL::route('chine_transport.chauffeurs.destroy', ['id' => '__ID__']) }}`.replace('__ID__', row.id);
            ...
            $('#confirmDeleteBtn').click(function() {
                $.ajax({
                    url: `{{ URL::route('chine_transport.chauffeurs.destroy', ['id' => '__ID__']) }}`.replace('__ID__', chauffeurIdToDelete),
                    type: 'DELETE',
                    data: {
