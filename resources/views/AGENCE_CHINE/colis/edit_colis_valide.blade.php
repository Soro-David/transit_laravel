...
<section class="p-4 mx-auto">
    ...
    @if(!$colis_recap_list->isEmpty())
        <div class="all-forms-container">
            ...
            {{-- FORMULAIRE GLOBAL                                               --}}
            {{-- ================================================================= --}}
            <form id="update-all-form" action="{{ route('chine_colis.valide.update') }}" method="POST">
                @csrf
                @method('PUT')
    ...
    $(document).ready(function() {
        ...
        function initializeProductAutocomplete(context) {
            $(context).find('.produit-autocomplete').each(function() {
                initAutocomplete($(this), "{{ route('chine_colis.recherche.auto') }}", 'Colis', productAutocompleteCallback);
            });
        }
        ...
        function initializeServiceAutocomplete() {
            initAutocomplete($('#service-container .service-autocomplete'), "{{ route('chine_colis.recherche.auto.service') }}", 'Service', serviceAutocompleteCallback);
        }
        ...
        $('#client_select').on('keyup', function() {
            ...
            $.ajax({
                url: "{{ route('chine_colis.clients.search') }}", // URL pour la recherche de clients
                dataType: 'json',
                data: { q: query },
                success: function(data) {
