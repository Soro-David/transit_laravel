...
@section('content')
    ...
    <script>
        ...
        axios.get("{{ route('ipmxangre_programme.data') }}")
        ...
        $(document).on('click', '.edit-programme-btn', function() {
            ...
            editForm.attr('action', `/IPMS_SIMEXCI_ANGRE/programmeipmxangre/update/${programmeId}`); // <-- Idem
            // Modifier l'URL pour la requête GET AVEC le préfixe /admin
            axios.get(`/IPMS_SIMEXCI_ANGRE/programmeipmxangre/edit/${programmeId}`) // <-- Ajouter le préfixe
                .then(response => {
        ...
        $('#editProgrammeForm').off('submit').on('submit', function(event) {
            ...
            axios.put(`/programmeipmxangre/update/${programmeId}`, formData)
                .then(response => {
                    return axios.get("{{ route('ipmxangre_programme.data') }}");
                })
                .then(response => {
                    programmesData.programmes = response.data.programmes;
                    updateTable();
        ...
        $('#addProgrammeModal form').off('submit').on('submit', function(event) {
            ...
            axios.post("{{ route('ipmxangre_programme.store') }}", $(this).serialize())
                .then(response => {
        ...
        $(document).on('click', '.delete-programme-btn', function() {
            ...
            if (confirm('Êtes-vous sûr de vouloir supprimer ce programme ?')) {
                axios.delete(`/IPMS_SIMEXCI_ANGRE/programmeipmxangre/delete/${programmeId}`) // <-- Préfixe ajouté
                    .then(response => {
