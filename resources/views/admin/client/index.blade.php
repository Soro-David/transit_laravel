...
@section('content')
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('client.sendGlobalMessage') }}" method="POST">
                @csrf
            ...
            @foreach($uniqueClients as $client)
                <tr>
                    ...
                    <td>
                        ...
                        <form action="{{ route('client.toggleActivation', ['id' => $client->id]) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PUT')
                            <button type="submit" 
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#sendMessageModal').on('show.bs.modal', function (event) {
                ...
                modal.find('#sendMessageForm').attr('action', '{{ route('client.sendMessage', ['id' => '__ID__']) }}'.replace('__ID__', clientId));
            });
