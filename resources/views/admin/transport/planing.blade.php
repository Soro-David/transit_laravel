...
<section class="p-4 mx-auto">
    <div class="card mx-auto" style="max-width: 1200px;">
        <div class="card-body">
            <form action="{{route('transport.store.plannification')}}" method="post" class="form-container">
                @csrf
...
$(document).ready(function() {
    ...
    $("#reference_colis").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: '{{ route('transport.reference.auto', ['query' => '__QUERY__']) }}'.replace('__QUERY__', request.term),
                type: 'GET',
                dataType: 'json',
