...
@section('content')
    <div class="container">
        ...
        <div class="modal fade" id="addProgrammeModal" tabindex="-1" role="dialog" aria-labelledby="addProgrammeModalLabel" aria-hidden="true">
            <!-- MODIFICATION: Remplacement de modal-lg par modal-xl pour agrandir la modale -->
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form method="post" action="{{ route('programme.store') }}">
                            @csrf
                            <!-- MODIFICATION: Ajout de la structure en grille (row/col) -->
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
        ...
        <div class="modal fade" id="editProgrammeModal" tabindex="-1" role="dialog" aria-labelledby="editProgrammeModalLabel" aria-hidden="true">
            <!-- MODIFICATION: Ajout de modal-xl pour agrandir la modale -->
            <div class="modal-dialog modal-xl" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-primary text-white">
                    ...
                    <div class="modal-body">
                        <form id="editProgrammeForm" method="POST">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="edit_reference_colis" class="form-label">Référence Colis :</label>
                                        <input type="text" name="reference_colis" class="form-control" id="edit_reference_colis" data-modified="false">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
    ...
    <script>
        ...
        $('#add-programme-entry').on('click', function() {
            ...
            // MODIFICATION: Mise à jour du template JS pour utiliser la grille
            let newEntry = `
                <div class="programme-entry">
                    <hr>
                    <div class="row">
        ...
        $(document).on('input', '.reference_colis', function() {
            ...
            const parentEntry = $(this).closest('.programme-entry');
            ...
            if (selectedReference.length >= 3) {
                axios.get(`/admin/colis/getColisInfo/${selectedReference}`)
                    .then(response => {
                        ...
                        if (colis) {
                            parentEntry.find('input[name="nom_expediteur[]"]').val(colis.expediteur.nom + ' ' + colis.expediteur.prenom);
                            parentEntry.find('input[name="Adresse_expedition[]"]').val(colis.expediteur.lieu_expedition);
                            parentEntry.find('input[name="tel_expediteur[]"]').val(colis.expediteur.tel);
                            parentEntry.find('input[name="nom_destinataire[]"]').val(colis.destinataire.nom + ' ' + colis.destinataire.prenom);
                            parentEntry.find('input[name="tel_destinataire[]"]').val(colis.destinataire.tel);
                            parentEntry.find('input[name="Adresse_destination[]"]').val(colis.destinataire.lieu_destination);
                        } else {
                            parentEntry.find('input[name^="nom_expediteur"], input[name^="Adresse_expedition"], input[name^="tel_expediteur"], input[name^="nom_destinataire"], input[name^="tel_destinataire"], input[name^="Adresse_destination"]').val('');
                        }
                    })
                    .catch(error => {
        ...
        $(document).on('click', '.edit-programme-btn', function() {
            ...
            axios.get(`/admin/programme/edit/${programmeId}`)
                .then(response => {
                    ...
                    // MODIFICATION: Utilisation de l'ID correct pour le champ de référence
