    ...
    const onScanSuccess = (decodedText) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // Vérifie si la réponse contient des messages à afficher
                if (response && response.messages && Array.isArray(response.messages) && response.messages.length > 0) {
                    ...
                    resultElement.innerText = "Aucun message spécifique n'a été retourné.";
                }
            },
            error: function (error) {
                console.error("Erreur lors du chargement :", error);

                // Vérifie si la réponse d'erreur contient des messages à afficher
                if (error.responseJSON && error.responseJSON.messages && Array.isArray(error.responseJSON.messages) && error.responseJSON.messages.length > 0) {
                    resultElement.innerText = error.responseJSON.messages.join("\n");
                } else {
                    resultElement.innerText = "Une erreur s'est produite lors de la mise à jour de l'état du colis.";
                }
            },
        });
